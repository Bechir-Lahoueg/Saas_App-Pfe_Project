...
const MediaManager = () => {
  ...
  const handleUpload = async (e) => {
    ...
    try {
      ...
      showSuccess(`${selectedType} téléchargé avec succès`);
  ...
  const confirmDelete = async (id, type) => {
    ...
    modalContainer.className = 'fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/30 backdrop-blur-sm';
      ...
      <div className="max-w-6xl mx-auto">
        ...
        >
          ...
          <p className="text-gray-600 text-lg max-w-2xl mx-auto">
            Gérez tous vos médias en un seul endroit pour une expérience utilisateur optimale
          </p>
        </motion.div>
          ...
          {loading && (
            ...
            >
              <motion.div 
                ...
                className="bg-white rounded-2xl shadow-2xl p-8 flex flex-col items-center max-w-md w-full mx-4"
              >
                ...
                <div className="text-center">
                  ...
                  <h3 className="text-xl font-medium text-gray-800 mb-2">
                    {uploadProgress < 100 ? (
                      selectedType === 'video' ? 'Upload de la vidéo en cours...' : 'Upload de l\'image en cours...'
                    ) : (
                      'Traitement en cours...'
                    )}
                  </h3>
                  ...
                  <p className="text-gray-500 mb-4 max-w-xs mx-auto">
                    {uploadProgress < 100 ? (
                      selectedType === 'video' ? 
                        'La vitesse d\'upload peut varier selon la taille du fichier et votre connexion.' :
                        'Votre média est en cours d\'envoi vers nos serveurs.'
                    ) : (
                      'Presque fini! Nous préparons votre média pour l\'affichage.'
                    )}
                  </p>
                  ...
                  <div className="w-full h-2 bg-gray-200 rounded-full mt-4 overflow-hidden">
                    <motion.div 
                      className="h-full bg-gradient-to-r from-blue-500 via-blue-400 to-indigo-600"
                      initial={{ width: 0 }}
                  ...
                  {uploadProgress < 100 && (
                    <motion.div 
                      ...
                      transition={{ delay: 1 }}
                    >
                      {uploadProgress < 30 && "Démarrage de l'upload..."}
                      {uploadProgress >= 30 && uploadProgress < 70 && "Ça avance bien !"}
                      {uploadProgress >= 70 && uploadProgress < 90 && "Presque terminé..."}
                      {uploadProgress >= 90 && uploadProgress < 100 && "Quelques secondes encore..."}
                    </motion.div>
                  )}
                  
                  {/* Astuce */}
                  <motion.div 
                    className="mt-6 text-xs text-blue-600 bg-blue-50 p-3 rounded-lg"
                    initial={{ opacity: 0 }}
                    animate={{ opacity: 1 }}
                    transition={{ delay: 2 }}
                  >
                    <span className="font-medium">Astuce:</span> {" "}
                    {selectedType === 'logo' && "Un bon logo devrait être lisible même en petit format."}
                    {selectedType === 'banner' && "Une bannière de qualité attire l'attention des utilisateurs dès leur arrivée."}
                    {selectedType === 'video' && "Les vidéos courtes ont souvent un meilleur taux d'engagement."}
                    {selectedType === 'photo' && "Des photos de qualité professionnelle améliorent la perception de votre marque."}
                  </motion.div>
                </div>
              </motion.div>
            </motion.div>
        ...
        <div className="bg-white/80 backdrop-blur-md rounded-2xl shadow-xl overflow-hidden border border-white/20">
          ...
          <div className="flex border-b border-gray-200">
            <button
              ...
              className={`py-4 px-8 font-medium transition-all duration-300 ${activeTab === 'upload' 
            ...
            <button
              ...
              className={`py-4 px-8 font-medium transition-all duration-300 ${activeTab === 'gallery' 
                ? 'border-b-2 border-blue-500 text-blue-600 bg-blue-50/50' 
                : 'text-gray-500 hover:text-gray-700 hover:bg-gray-50/50'}`}
            >
              <span className="flex items-center">
              ...
              {activeTab === 'upload' && (
                ...
                >
                  <div className="flex items-center mb-6">
                    ...
                    <h2 className="text-2xl font-bold text-gray-800">Ajouter un média</h2>
                  </div>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                      <div>
                        ...
                        <div className="relative">
                          ...
                          <option value="photo">Photos ({medias.photo?.length || 0}/5)</option>
                          </select>
                        ...
                        {((selectedType === 'logo' && mediaExists('logo')) || 
                            ...
                            <div className="flex items-start">
                              ...
                              <div>
                                <h3 className="text-sm font-medium text-amber-800">
                                  Remplacement de {selectedType === 'logo' ? 'logo' : selectedType === 'banner' ? 'bannière' : 'vidéo'}
                                </h3>
                                <p className="text-xs text-amber-600 mt-1">
                                  Vous avez déjà un(e) {selectedType === 'logo' ? 'logo' : selectedType === 'banner' ? 'bannière' : 'vidéo'} actif(ve).
                                  Le téléchargement d'un nouveau fichier remplacera l'existant.
                                </p>
                                <div className="mt-2 flex space-x-2">
                                  ...
                                  <button 
                                    type="button"
                                    onClick={() => {
                                      if (selectedType === 'logo' && medias.logo[0]) {
                                        handleDelete(medias.logo[0].id, 'Logo');
                                      } else if (selectedType === 'banner' && medias.banner[0]) {
                                        handleDelete(medias.banner[0].id, 'Bannière');
                                      } else if (selectedType === 'video' && medias.video[0]) {
                          ...
                          <div className="mt-3 p-3 bg-blue-50 rounded-lg border border-blue-100">
                            <h3 className="text-sm font-medium text-blue-800 mb-1">Recommandations</h3>
                            <p className="text-xs text-blue-600">
                              {selectedType === 'logo' && 'Format: PNG ou SVG avec fond transparent. Taille recommandée: 200x80px'}
                              {selectedType === 'banner' && 'Format: JPG ou PNG haute résolution. Taille recommandée: 1920x480px'}
                              {selectedType === 'video' && 'Format: MP4, WebM. Durée maximale recommandée: 1 minute'}
                              {selectedType === 'photo' && `Format: JPG ou PNG haute résolution. Aspect ratio de 16:9 recommandé. (${medias.photo?.length || 0}/5 photos)`}
                            </p>
                          </div>
                        )}
                      </div>
                      ...
                      <div>
                        ...
                        <div 
                          ...
                          className={`mt-1 flex justify-center px-6 pt-5 pb-6 border-2 ${
                            isDragActive ? 'border-blue-400 bg-blue-50' : 'border-gray-300 border-dashed'
                          } rounded-lg hover:border-blue-400 transition-colors duration-200 cursor-pointer group`}
                        >
                          <div className="space-y-2 text-center">
                            <svg 
                              className={`mx-auto h-12 w-12 ${isDragActive ? 'text-blue-500' : 'text-gray-400'} group-hover:text-blue-500 transition-colors duration-200`} 
                              stroke="currentColor" 
                              fill="none" 
                              viewBox="0 0 48 48"
                            >
                            ...
                            <div className="flex text-sm text-gray-600 justify-center">
                              <label className="relative cursor-pointer bg-white rounded-md font-medium text-blue-600 hover:text-blue-500 focus-within:outline-none">
                                <span className="px-2 py-1.5 rounded hover:bg-blue-50 transition-colors duration-200">
                                  {isDragActive ? 'Déposez le fichier ici' : 'Télécharger un fichier'}
                                </span>
                                <input 
                                  {...getInputProps()}
                                  id="file-input" 
                              ...
                              {!isDragActive && <p className="pl-1">ou glisser-déposer</p>}
                            </div>
                            <p className="text-xs text-gray-500">
                              {selectedType === 'video' ? 'MP4, WebM, Ogg' : 'PNG, JPG, GIF jusqu\'à 10MB'}
                            </p>
                          </div>
                        </div>
                      </div>
                ...
                >
                  ...
                  <div className="bg-white/60 backdrop-blur-sm rounded-xl overflow-hidden shadow-md border border-white/80">
                    <div className="flex overflow-x-auto scrollbar-hide">
                      ...
                      {Object.keys(mediaTypeLabels).filter(key => key !== 'all').map((type) => (
                        <button
                          key={type}
                  ...
                  {/* Gallery content */}
                  <AnimatePresence mode="wait">
                    {activeMediaType === 'all' || activeMediaType === 'logo' || activeMediaType === 'banner' ? (
                      <motion.div
                        key="logo-banner" 
                        initial={{ opacity: 0 }}
                        animate={{ opacity: 1 }}
                      ...
                      >
                        {/* Logo */}
                        {(activeMediaType === 'all' || activeMediaType === 'logo') && (
                          <motion.div 
                            ...
                            <div className="bg-white/80 rounded-lg p-6 flex items-center justify-center min-h-[180px] border border-gray-100 shadow-inner">
                              {medias.logo.length > 0 ? (
                                ...
                                <div className="text-center">
                                  ...
                                  <p className="text-gray-500 mt-2">Aucun logo téléchargé</p>
                                  <button 
                                    onClick={() => {
                                      setActiveTab('upload');
                        ...
                        {/* Bannière */}
                        {(activeMediaType === 'all' || activeMediaType === 'banner') && (
                          <motion.div 
                            className="bg-white/70 backdrop-blur-sm p-6 rounded-xl shadow-lg transform transition-all duration-300 hover:shadow-xl border border-white/50 relative overflow-hidden"
                            whileHover={{ translateY: -5 }}
                          >
                            ...
                            <div className="bg-white/80 rounded-lg p-4 flex items-center justify-center min-h-[180px] border border-gray-100 shadow-inner">
                              {medias.banner.length === 0 ? (
                                <div className="text-center">
                                  ...
                                  <p className="text-gray-500 mt-2">Aucune bannière téléchargée</p>
                                  <button 
                                    onClick={() => {
                                      setActiveTab('upload');
                                ...
                                <div className="w-full">
                                  ...
                                  <div className="flex justify-between items-center mt-4">
                                    <p className="text-xs text-gray-500">
                                      Ajouté le {new Date(medias.banner[0].createdAt || Date.now()).toLocaleDateString()}
                                    </p>
                                    <button 
                                  ...
                                  <div className="flex items-center mt-2 bg-gray-50 p-3 rounded-lg">
                                    ...
                                    <p className="text-xs text-gray-600">Pour changer de bannière, supprimez d'abord celle-ci puis ajoutez-en une nouvelle.</p>
                                  </div>
                                </div>
                              )}
                            </div>
                        ...
                        <div className="bg-white/80 rounded-lg p-6 flex items-center justify-center border border-gray-100 shadow-inner">
                          {medias.video.length > 0 ? (
                            ...
                            <div className="text-center py-10">
                              ...
                              <p className="text-gray-500 mt-3">Aucune vidéo téléchargée</p>
                              <button 
                                onClick={() => {
                                  setActiveTab('upload');
                    ...
                    {/* Photos */}
                    {(activeMediaType === 'all' || activeMediaType === 'photo') && (
                      <motion.div 
                        key="photos"
                        initial={{ opacity: 0 }}
                        animate={{ opacity: 1 }}
                        ...
                        <div className="bg-white/80 rounded-lg p-6 border border-gray-100 shadow-inner">
                          {medias.photo.length > 0 ? (
                            ...
                            <div className="flex flex-col items-center justify-center py-10">
                              ...
                              <p className="text-gray-500 mt-3">Aucune photo téléchargée</p>
                              <button 
                                onClick={() => {
                                  setActiveTab('upload');
                                  setSelectedType('photo');
                    ...
                    {/* Empty state */}
                    {Object.values(medias).every(arr => arr.length === 0) && activeMediaType === 'all' && (
                      <motion.div 
                        initial={{ opacity: 0 }}
                        animate={{ opacity: 1 }}
                      ...
                      >
                        ...
                        <h3 className="text-xl font-medium text-gray-700 mb-2">Aucun média disponible</h3>
                        <p className="text-gray-500 max-w-md mb-6">Commencez par ajouter un logo, une bannière, des photos ou une vidéo pour personnaliser votre espace.</p>
                        <button 
                          onClick={() => setActiveTab('upload')} 
        ...
        <div className="mt-10 text-center text-sm text-gray-500">
          <p>Utilisez des médias de haute qualité pour une meilleure présentation.</p>
        </div>
      </div>
    </div>
  );
